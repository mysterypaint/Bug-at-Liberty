...
function draw_level_data(_level_data_grid, _is_collision_layer) {
	...
	if (instance_exists(level_data_obj)) {
		for (var _y = _ty_start; _y <= _ty_end; _y++) {
			for (var _x = _ty_start; _x < _tx_end; _x++) {
				if (_y < 0 || _x < 0 || _y >= LevelData.map_height || _x >= LevelData.map_width)
					continue;
				...
				if (!_is_collision_layer) {
					...
					for (var _i = 0; _i < _num_gids; _i++) {
				        ...
				        {
				            _tileset = level_data_obj.tilesets[| _i];
					...
					var _this_tile = tilemap_get(LevelData.collision_layer_tiles, _x, _y);
					var _tile_index = tile_get_index(_this_tile);
					if (_tile_index >= 1 && Game.debug) {
						draw_set_alpha(0.4);
						draw_sprite(sprWall, 0, _x * _ts, _y * _ts);
						draw_set_alpha(1);
					}
